...
import { firefox, Browser, BrowserContext, Route, Request as PlaywrightRequest, Page } from 'playwright';
// @ts-ignore — camoufox-js does not ship type declarations yet
...
const initializeBrowser = async () => {
  // Launch Camoufox in-process via Playwright's firefox driver, using the
  // launch options produced by camoufox-js (patched Firefox binary path,
  // fingerprint flags, etc.). The actual driver is stock Playwright.
  browser = await firefox.launch({
    ...(await launchOptions({ headless: true })),
  });

  browser.on('disconnected', () => {
    console.warn('Camoufox browser disconnected; will relaunch on next request');
